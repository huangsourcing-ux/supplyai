...
All required fields from `apps/web/.env.example` must have real staging values before deployment. The database is the Railway staging PostgreSQL database, Clerk uses a Development instance, and the Web/Payload R2 credentials are limited to Object Read/Write on `chinasupply-staging-media`; `R2_MEDIA_BUCKET`, `R2_PREFIX=staging`, and `R2_CDN_BASE_URL=https://cdn-staging.chinasupply.ai` identify that media boundary. Remote Cloudflare R2 derives the S3 endpoint from `R2_ACCOUNT_ID`; omit `R2_ENDPOINT` outside local/test S3-compatible environments. The public media bucket CORS policy must allow `PUT` from the exact Web origin with `Content-Type` and `Content-Length`; CMS signing requests remain same-origin and Payload-authenticated. Migration-only jobs intentionally omit R2 values and cannot upload. The production Web project is intentionally deferred to M5-T9.
