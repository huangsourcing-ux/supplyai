...
当前计划起点是 `M4-T1b`；M0-M3 工程任务已收口，M4-T0 已产出 F-1/F-11.2/N-1/N-6 的地图体验诊断和两套方向稿，Owner 在同一 PR 明确选择 `Keep baseline`；M4-T1a 已完成 App 共享底图、MAP-1/2/3 数据源、zoom 分层、防抖/abort、错误重试与 attribution。M4-T1 父项仍须完成 M4-T1b 的聚合、底部卡片和交互后才可勾选。Owner 另行要求把与官方 Streets v4 差异过大的手工底图更正为去密钥后的官方样式快照，该工作已按 M2-T9 独立修复交付。M0-T0 中 Apple Developer、Google Play Console 与商店标识保留等人工前置仍未完成，不得伪造或勾选；外部账号、真机测试、数据录入和人工验收仍属于人工前置或人工门禁。
