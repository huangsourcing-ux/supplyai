...
当前计划起点是 `M5-T2`；M0-M3 工程任务已收口，M4-T0 已产出 F-1/F-11.2/N-1/N-6 的地图体验诊断和两套方向稿，Owner 在同一 PR 明确选择 `Keep baseline`；M4-T1 已完成 App 共享底图、MAP-1/2/3 数据源、zoom 分层、防抖/abort、聚合放大、产业带/工厂卡片、truncated、错误重试、attribution 与共享 `map_moved` facade 接线，并使用真实受限 iOS/Android MapTiler key + canonical staging API 完成双端 Simulator/Emulator 交互 smoke；M4-T2a 已完成 F-3.1～F-3.6 App 搜索与 F-1.6 一级类目 chips，包括 300ms 搜索/500ms 类目筛选防抖、旧 MAP 请求取消、二级精确筛选态、结果定位和即时卡片，并完成双端真实 key + staging API smoke。M4-T2b/T2c 已分别完成产业带详情与工厂详情路由、A-2/A-3/A-5、小地图、安全 Markdown、cursor 列表、图片/可选信息、安全联系方式、relatedFactories 和完整状态；双端受限 key + canonical staging API 主路径与固定 fixture/自动化分支验收均通过，按开发计划 v1.3 收口。M4-T3a 已实现 Map/Account Tabs、邮箱验证码注册/登录与 email-code MFA/client trust、Google browser SSO、英语 locale、登出/删除/401 统一 session cleanup 以及 A-9/A-10；双端 Simulator/Emulator 的一次性邮箱完整生命周期、Clerk 删除、core tombstone、favorites 清零和 webhook 均已通过，Google 浏览器打开/取消及成功/取消/失败单测通过。真实 Google 成功 session 因缺测试身份未执行且不记为通过；Owner 于 2026-07-28 明确批准现有验收边界，并在 `createClerkTokenVerifier` 默认/显式 `web-only`、`web-or-native` 和 Admin guard 省略 policy 的回归保护补齐后批准收口。M4-T3b 已完成 F-5.1～F-5.3/F-2.3：Map/Saved/Account Tabs、受限登录回跳、按 Clerk 用户隔离的 cursor 收藏缓存、详情收藏、Saved 两类列表取消、完整状态及聚焦重验均已落地；双端真实受限 key + canonical staging API 已通过邮箱登录回跳、两类收藏/列表/详情/取消、同账户跨端同步、失败/Retry 与 401 cleanup，临时收藏和账户已清理。M4-T4 已完成 F-10.1：Tabs 为 Map/Explore/Saved/Account，A-7 一级类目网格、A-1 精确类目 cursor 产业带列表、详情往返、完整状态和 icon 回落均已落地；双端真实受限 key + canonical staging API 通过九类网格、Electronics/Home Textiles 列表、Lighting 空态和详情往返，Android 另通过真实断网→Retry，第二页 cursor 因 canonical staging 无数据仅用固定 fixture 覆盖，未修改线上数据。M4-T5 已完成 F-6.3：双端平台限定导航按钮复用 M0-T9 `buildNavUrl`，高德/百度支持 Web 回落，双重失败可恢复，每次点击仅调用一次 `navigation_clicked` facade；固定自动化覆盖平台集合、WGS-84 URL、主链接/回落/双重失败及埋点去重。Owner 于 2026-07-29 明确确认已完成 F-6.4 的 13 项 Release 真机矩阵、全部终点误差 `<50m` 并批准勾选。M4-T6 已完成 canonical staging Maestro 核心路径：地图 ready、搜索到产业带/工厂、匿名/登录 Saved、两类收藏/取消、双平台导航按钮集合、登出/重登和二次确认删除均在 iOS 26.5 Simulator 与 Android API 36 Emulator 跑绿；失败后 `onFlowComplete` 清理和只读聚合核验确认 Clerk 删除、core tombstone、favorites 清零及删除 webhook 成功，精确 OPPO WGS-84 导航 URL 继续由固定单测断言。设备/系统、构建产物与地图 App 版本等 M4-T5 逐项原始记录未随 Owner 确认提交仓库，仓库只记录验收结论而不补造字段。M4-T7 已按 Owner 批准的离线口径完成 F-8.1/F-11.1/F-11.4：iOS 原生 Sign in with Apple 与 Google 同显同隐、Production 配置门禁、登录前后 canonical privacy/terms、无追踪 App Privacy manifest、required-reason API 原因、Android 权限/备份加固及 Apple/Google 离线声明包均已落地。M4-T8 经 Owner 批准改为商店门禁迁移任务并已收口：Production iOS Bundle ID/Android package 候选改为尚未保留的 `ai.chinasupply.mobile`，scheme 保留 `chinasupply`；Apple Developer/Play Console、正式标识符保留、Apple capability、Clerk Production/Native Application 配置迁到 M5-T9，商店表单、真实 Apple 成功登录、TestFlight/Play 内测与 Production Submit 迁到 M5-T10。本次未访问商店控制台、未接受协议或付款、未触发 EAS、未发布内测包；M4 出口只依据既有功能、Maestro、双端核心路径和导航真机验收关闭，不得反向宣称商店验收通过。M5-T1 已完成 ADM-1/3 Create、ADM-6 R2 上传链与 Admin 写限流加固，并在 exact main/staging 部署后使用 Owner 授权真实 JPEG 对 Nantong Jinkanghong 工厂完成 presign→PUT→PATCH/HEAD→`/ops` 重新 verify→A-5/CDN；公开响应无 `objectKey` 字段且 CDN 内容哈希与源图一致。canonical published 数据现已有 1 家带真实图片的工厂，仍没有 description/stats，单带最多 5 家且缺经 SOP 审核的 Phone/Email/WeChat；M5-T2 负责 `/ops` 新建、地图选点、图片上传 UI 及联系方式/拨号器复验。21+ 同产业带真实 cursor 仅在 M5 数据自然满足时追加且不人为造数，Unverified 不要求长期发布样本。Mobile 真实 PostHog adapter + Consent 必须另行批准并先修订开发计划，不得夹带。Owner 另行要求把与官方 Streets v4 差异过大的手工底图更正为去密钥后的官方样式快照，该工作已按 M2-T9 独立修复交付。外部账号、生产凭据、物理真机测试、数据录入和人工验收仍属于 M5 人工前置或人工门禁；未实际完成者不得伪造或勾选。
...
- 展示、营销、占位和编辑配图默认由代理直接生成，不要求 Owner 先上传素材或逐张确认版权。生成图须避免未授权品牌和可识别真人身份，使用准确 alt，并标注为 AI-generated illustration；不得把它当作真实厂区、制造能力、产品、资质、身份或数据核验 SOP 的事实证据。只有任务明确要求纪实真实性，或图片本身承担事实证明时，才需要可追溯真实素材。该规则不放宽 verified/curated、来源台账或 production 迁移门禁。
