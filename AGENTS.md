...
当前计划起点是 `M5-T7`。M0-M4 工程任务已按开发计划收口；M4-T8 只完成经 Owner 批准的商店门禁迁移，Apple Developer/Play Console、正式标识符保留、Apple/Clerk Native Production 配置仍在 M5-T9，商店表单、真实 Apple 成功登录、TestFlight/Play 内测与 Production Submit 仍在 M5-T10，不得反向宣称商店验收通过。Owner 于 2026-07-31 授权为替换无效旧 `www` 站点，在 M5-T7 中提前执行 M5-T8a/M5-T9 的 Web production 前置与可能计费的 Railway/Cloudflare/Clerk 资源，但各任务只有完整原门禁满足后方可勾选；Apple/商店/Native/EAS 不提前。Owner 同时确认当前 `www` 仅作非商业研发/验收，允许独立正式域名受限 MapTiler Free R&D key；首次商业使用、收费、商业发布或商店 Production Submit 前仍必须升级 Flex 并完成三平台 production keys/账单门禁。Production Cloudflare 双桶、媒体域名、限定 apex/`www` 的 CORS 和两只最小权限 R2 Account Token 已完成真实 S3 smoke；Web 密钥已保存为 Vercel Production-only sensitive variables，API/Worker 密钥已保存为 Railway production 共享变量。Owner 选择暂不付费并保留 Railway Trial；Railway 账户页显示 verified Trial、21 天及 USD 3.74 余额，但真实 production 项目创建托管 Redis 与直接 `redis:8.2.1` 镜像服务均被平台的 `Free plan resource provision/creation limit exceeded` 门禁拒绝，未创建任何新资源。当前 PostGIS 与变量预备可继续用于非商业研发，但此账户级门禁仍阻塞 Redis/API/Worker；不得通过删除或挪用 canonical staging 资源绕过，因此 M5-T7/M5-T9 均不得据此勾选。M5-T1 已完成 ADM-1/3 Create、ADM-6 R2 上传链与 Admin 写限流加固，并完成 canonical staging 真实图片链验收。M5-T2 已完成 `/ops` draft 新建、WGS-84 MapLibre 选点和完整媒体管理；Owner 独立 SOP 确认后，仅通过 `/ops` 写入、verify、publish Jinkanghong/Yayu 联系方式，双端复跑图片、Phone/Email/WeChat、规范化 `tel:` 与复制分支。Owner 批准的 AI-generated illustration 已在 canonical `/ops` 完成 presign→credential-free PUT→PATCH/HEAD→verification reset→reviewer acknowledgement/Verify→`next/image` 预览→A-5/CDN；公开响应无 `objectKey`，CDN MIME、字节数及 SHA-256 与生成源一致，未创建 synthetic entity。M5-T3 已在 `seed:real` 的类目 upsert 路径完成 name/aliases 实际变化检测，并通过可复用的 BullMQ `regenerate:search-text` job 与共享 `buildSearchText` fan-out 重算关联产业带/工厂。M5-T4 已完成 Payload `articles`/`media`、独立 R2 client upload、Lexical Cluster Card 与 `/guides` SSR/ISR，并完成 canonical staging 文章验收。M5-T5 已完成专用无坐标文件 → R2/BullMQ → 高德 → GCJ-02/WGS-84 → 非空 location upsert、验证状态重置与全链路自动化。M5-T6 已完成 PostgreSQL 17 custom archive → age 1.3.0 → staging 私有 R2、HEAD/manifest 完整性校验、滚动 30 天保留、`03:00 UTC` scheduler 与隔离恢复演练。21+ 同产业带真实 cursor 只在 M5 数据自然满足时追加且不人为造数，Unverified 不要求长期发布样本。Mobile 真实 PostHog adapter + Consent 必须另行批准并先修订开发计划，不得夹带。外部账号、生产凭据、物理真机测试、数据录入和人工验收仍属于后续 M5 人工前置或人工门禁；未实际完成者不得伪造或勾选。
...
- 基础设施：MapTiler Cloud（非商业开发/验收可用独立受限 Free key；商业 Production 必须 Flex）、Cloudflare R2/CDN、Clerk、Resend、Sentry、PostHog；Web/Payload 部署到 Vercel，API/Worker 部署到 Railway 美区。
...
- MapTiler Free 只允许用于非商业开发/测试/验收。Owner 已确认当前 `www.chinasupply.ai` 仍属非商业研发验收，因此可临时使用一只独立、仅限 apex/`www` 的 Free R&D Web key；禁止复用 staging key或将其解释为商业 production 授权。首次商业使用、收费、商业发布或商店 Production Submit 前必须升级 Flex、确认商业授权与账单上限、创建 Web/iOS/Android 三只受限 production key，并完成 Planet v4 TileJSON/PBF/glyph/sprite smoke。
