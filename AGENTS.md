...
当前执行起点是 `M4-T0`；M0-M3 工程任务已收口，但 M0-T0 中 Apple Developer、Google Play Console 与商店标识保留等人工前置仍未完成，不得伪造或勾选。M4-T0 只产出 F-1/F-11.2/N-1/N-6 的地图体验诊断和至少两套方向稿，不修改业务代码、共享 style JSON、API 或线上数据；未在同一 PR 记录 Owner 选定方向（或明确保留现基线）前，不勾选 M4-T0，不开工 M4-T1。外部账号、真机测试、数据录入和人工验收仍属于人工前置或人工门禁；代理不得伪造其完成状态。
