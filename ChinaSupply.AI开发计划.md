...
> 版本：**v1.8** ｜ Status: **Frozen / Approved for Execution** ｜ Next Action: **M5-T5** ｜ 日期：2026-07-30
...
- [ ] **M5-T5 导入增强**：`geocode:factories` 使用不含 `location` 的专用 CSV/JSON，经私有 R2 与 BullMQ 顺序调用高德；保留首个排序结果的 GCJ-02、转换并写入 WGS-84，再按 slug upsert与生成 search_text。核心表/API 的 location 保持必填/NOT NULL；每次成功写入均重置 `verified=false` 及全部验证审计字段，保留既有发布状态。实现 PR 只用本地假服务验收并保持本项未勾选；合并部署后必须以 staging Web Service key 和获批真实 `draft + unverified` canonical 样本完成不触及 production 的 smoke，再独立收口。（批量图片、任务 UI 与监控告警超出 PRD，移入 P1。）
