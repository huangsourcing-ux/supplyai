...
> 版本：**v1.9** ｜ Status: **Frozen / Approved for Execution** ｜ Next Action: **M5-T7** ｜ 日期：2026-07-31
> 依据：《ChinaSupply.AI技术栈-最终冻结版.md》+《ChinaSupply.AI产品PRD.md v1.7 Frozen》
...
> v1.7 变更：经 Owner 批准，冻结 M5-T4 Payload `articles`/`media`、R2 client upload、Lexical Cluster Card、`/guides` SSR/ISR 与两阶段交付验收。Payload 媒体采用独立的 Payload session + 同源/CSRF 预签名链，ADM-6 继续只处理产业带/工厂媒体；Cluster Card 只保存 cluster ID，不创建跨 Schema 关系。实现 PR 保持 M5-T4 未勾选；合并、迁移、exact-commit staging 部署及 canonical Payload 文章验收后，再由独立收口 PR 提交非敏感证据、勾选 M5-T4 并推进 M5-T5。未新增 `/api/v1`、OpenAPI/Orval、Mobile、埋点、主导航、sitemap 或 `/about`。
> v1.8 变更：经 Owner 批准，冻结 M5-T5 `geocode:factories` 为专用无坐标 CSV/JSON → 私有 R2 → BullMQ → 高德 → GCJ-02/WGS-84 → 非空 location upsert；无坐标状态不得进入核心表、Admin 或公开 API。实现 PR 使用本地假高德服务完成自动化但保持 M5-T5 未勾选；合并部署、配置 staging Web Service key，并对获批真实 `draft + unverified` canonical 样本完成不触及 production 的真实链路后，再由独立收口 PR勾选并推进 M5-T6。批量图片、任务 UI 与监控告警继续留在 P1。
> v1.9 变更：经 Owner 于 2026-07-31 明确授权，为替换 `www.chinasupply.ai` 的无效旧测试站，允许在 M5-T7 收口中提前执行 M5-T8a 与 M5-T9 的 **Web production 前置**，包括创建可能计费的 Railway/Cloudflare/Clerk/Vercel 生产资源、生产 migration dry-run/执行、canonical 内容迁移准备和 Web 切换；商店、Apple/Native、EAS 与 commercial Go-Live 门禁不提前。Owner 同时确认当前 `www` 仅用于非商业开发/验收，尚未商业化；因此允许在正式域名使用一只独立、仅限 `www.chinasupply.ai`/`chinasupply.ai` 的 MapTiler Free R&D key，禁止复用 staging key。此例外不构成商业 production 授权：首次商业使用、收费、对外商业发布或商店 Production 提交前，仍必须升级 Flex、确认商业授权/账单上限，并创建 Web/iOS/Android 三只正式受限 production key。M5-T7 只有真实切换并完成 production SEO/法律页/地图验收后方可勾选；M5-T8a/M5-T9 仍各自按完整原门禁勾选，不因前置部分完成而提前通过。
...
| Production | 独立生产 DB                       | Production instance | 两只独立 production bucket，空 prefix（M5-T7 Web 前置创建）          | 正式域名  |
...
- [x] **M5-T3 搜索列联动**：类目的 name/aliases 修改后触发 BullMQ `regenerate:search-text` 更新关联产业带与工厂（PRD 3.8）；publish/unpublish 的 MAP purge 已由 M2-T6 前置交付。**已在现有唯一类目写路径 `seed:real` 中按 name/aliases 实际变化检测并入队，Worker 复用 `buildSearchText` 重算目标类目及 `cluster_categories`/`factory_categories` 关联的全部产业带与工厂；无变化不入队，入队函数保持可复用。Testcontainers 真实 PostGIS + Redis + MinIO 验证首次 canonical seed 仅识别 1 个既有变化类目并完成 1/90/90 fan-out，第二次幂等 seed 的 `searchTextRegeneration` 为 null。未新增 Admin 类目接口、未修改 `/ops`、公开/Admin API 契约、数据库 schema 或 MAP purge。Next Action 推进到 M5-T4。**
- [x] **M5-T4 Payload 文章**：F-7.1 + `/guides`（F-10.2）+ 文章内产业带卡片。实现 Payload-owned `articles`/`media`、draft/version 生命周期、显式 CMS migration、R2 client upload 与上传后 HEAD 复验；Lexical Cluster Card 只保存 21 位 cluster ID，publish 阻止 unpublished 引用，运行时 MAP-1 失败或引用失效显示 unavailable。`/guides` 与详情仅 SSR/ISR 读取 `published + en`，覆盖 canonical/hreflang/description/article OG。**实现 PR #89 与兼容修复 PR #90–#93 已合并；final main commit `9c646aab48704216adc96d9f2765c5146ce89662` 通过 CI run `30551758036`、CMS/Core migration、Staging Release Gate 与 Vercel exact-commit 部署。canonical Payload Admin 已完成 PNG presign → credential-free R2 PUT → Media HEAD/create，并发布指定英语文章与 Dongguan Cluster Card；`/guides`/详情 SSR/ISR、155 字 description、canonical/hreflang/Article OG、AI 图片标注、Dongguan 链接、匿名 CMS API `403` 及源/CDN MIME/字节数/SHA-256 一致均通过。两条失败上传孤儿对象已通过 Payload lifecycle 精确删除并复验 `404`，canonical Media 保持 `200`。证据见 `docs/operations/m5-t4-payload-articles.md`；文章仅保留 canonical staging，未触及 production。Next Action 推进到 M5-T5。**
- [x] **M5-T5 导入增强**：`geocode:factories` 使用不含 `location` 的专用 CSV/JSON，经私有 R2 与 BullMQ 顺序调用高德；保留首个排序结果的 GCJ-02、转换并写入 WGS-84，再按 slug upsert与生成 search_text。核心表/API 的 location 保持必填/NOT NULL；每次成功写入均重置 `verified=false` 及全部验证审计字段，保留既有发布状态。**实现 commit `6a55259203767fe85b5bd22c569d3cdf8fbc065a` 完成严格 schema、CLI、R2/BullMQ、高德 client、GCJ-02/WGS-84、幂等 upsert、验证状态重置、敏感信息保护及 Testcontainers 全链路；Owner 于 2026-07-30 明确确认已按批准范围完成并验证 staging 验收，并指示在同一 PR 勾选后合并。仓库只记录该人工验收结论，不补造未提供的部署 ID、job ID、样本 slug 或私有报告字段。M5-T5 勾选，Next Action 推进到 M5-T6。**（批量图片、任务 UI 与监控告警超出 PRD，移入 P1。）
- [x] **M5-T6 备份**：F-9.3 每日加密 pg_dump → R2、30 天保留；**恢复演练一次并记录**。**实现 PR #96 已合并为 main commit `30b112be98c4e6e5efdcd6a5b427a6b322b58eea`，通过 CI、CMS/Core migration 与 Staging Release Gate，并由 canonical staging API/Worker exact-commit Docker 部署。Worker 仅配置公开 age recipient 与备份开关；持久化 BullMQ scheduler 为 `postgres-backup-daily-v1`、`0 0 3 * * *` UTC，下一次为 `2026-08-01T03:00:00.000Z`。真实 staging 手动备份生成 166813-byte age 密文及相邻 manifest，R2 HEAD 与 manifest SHA-256 一致；本机只读挂载 Owner 专用私钥，恢复到一次性独立 PostGIS 17 服务。源库/恢复库均为 PostgreSQL 17.5、PostGIS 3.5.2，21 个非扩展用户表、293 行、逐表计数摘要完全一致，Drizzle/Payload migration 分别为 1/2 行。临时服务与 TCP proxy 已移除，独立 volume 已进入平台待删除窗口，无本地密文，未触及 production。证据见 `docs/operations/m5-t6-encrypted-backups.md`；Next Action 推进到 M5-T7。**
- [ ] **M5-T7 静态页与 SEO**：创建 /about；在 production **复核**已上线的 /privacy、/terms（M3-T7）与地图 attribution（M2/M4 已实现，勿重写）；sitemap；空状态/骨架屏与 SEO 核查（F-10.4 / N-2）。**Owner 于 2026-07-31 授权本任务提前完成 M5-T8a/M5-T9 的 Web production 前置并替换无效旧站；只有新仓库 exact commit 实际接管 `www`，上述 production 路由、动态 sitemap、canonical/hreflang、地图 attribution 与 Lighthouse SEO 验收全部通过后才可勾选。**
...
- [ ] **M5-T8a 生产内容迁移（上线数据来源，唯一实质缺口的补齐项）**：数据三分类贯穿全程——测试/合成、真实未验证、已验证可迁移（curated）。staging 中 synthetic/test 数据使用独立 namespace（slug 前缀或标记），永不导出；以干净 CSV/JSON 为 canonical dataset，只导出 **verified 且 curated** 的 clusters/factories；生成导出 manifest（记录数、slug、校验和、R2 objectKey）；被引用图片复制到 production bucket/prefix；导入 production 保持 draft，production admin 核对 manifest + 抽样后 publish；导入后校验记录数、对象存在性与校验和；Payload articles/media 单独制定导入或生产重录方案。**Owner 已授权在 M5-T7 中提前实现并执行此链，但只有 manifest、媒体复制/HEAD、production Draft 导入、记录数/摘要复核和 production admin 抽样/发布全部完成后才勾选。**
- [ ] **M5-T9 Production Cutover + 商店账号前置**：先完成 Apple Developer/Google Play Console 注册与人工协议/2FA，验证并保留 Owner 选定的 `ai.chinasupply.mobile` iOS Bundle ID/Android package；任一平台不可用时停止并先修订批准文档。启用 Apple Sign in capability，创建 Clerk Production 与 Native Application/Apple connection。**Web production 前置经 Owner 授权可在 M5-T7 提前执行：创建/核对生产 DB、Redis、R2、Cloudflare、Clerk Production 与 Vercel，生产 migration dry-run/执行，部署 API/Worker/Web、smoke 与回滚记录；提前完成只计入证据，不提前勾选 M5-T9。** MapTiler 的 commercial 门禁调整为：当前 `www` 非商业研发/验收期间可使用一只独立、按正式 Web 域名限制的 Free R&D key，并完成 Planet v4 TileJSON、PBF、glyph、sprite smoke；staging key 仍不得复用。首次商业使用、收费、商业发布或商店 Production Submit 前，必须升级 Flex、确认商业授权/账单上限，并创建按正式 Web 域名、iOS Bundle ID、Android package 分别限制的三只 production key。最后验证 staging 数据未流入 production，并保留上线前备份、回滚命令与上一版本；Apple/商店/Native 前置仍不得跳过。
