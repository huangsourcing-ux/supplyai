...
> 版本：**v1.7** ｜ Status: **Frozen / Approved for Execution** ｜ Next Action: **M5-T4** ｜ 日期：2026-07-30
...
- [ ] **M5-T4 Payload 文章**：F-7.1 + `/guides`（F-10.2）+ 文章内产业带卡片。实现 Payload-owned `articles`/`media`、draft/version 生命周期、显式 CMS migration、R2 client upload 与上传后 HEAD 复验；Lexical Cluster Card 只保存 21 位 cluster ID，publish 阻止 unpublished 引用，运行时 MAP-1 失败或引用失效显示 unavailable。`/guides` 与详情仅 SSR/ISR 读取 `published + en`，覆盖 canonical/hreflang/description/article OG。实现 PR 合并并通过 main CMS/Core migration、Staging Release Gate 和 Vercel exact-commit 部署后，以 canonical Payload Admin 发布指定英语验收文章和 Dongguan card，完成浏览器上传、SSR/metadata/CDN 完整性与匿名 CMS API 拒绝验收；再由独立收口 PR 提交非敏感证据并勾选。
